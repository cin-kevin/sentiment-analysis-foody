...
from crawl.items import FoodyCommentItem, FoodyItem
...
class FoodyLoginSpider(scrapy.Spider):
    ...
    def start_scrapping(self, response):
        ...
        for url in self.start_urls:
            yield scrapy.Request(
                url=url, cookies=cookies_dict, callback=self.parse_restaurant
            )
    ...
    def parse_restaurant(self, response):
        ...
        total_ = response.xpath(
            "//div[@class='result-status-count']/div/span/text()"
        ).extract()[0]
        total_page_ = int(int(re.sub(r"\.", "", total_)) / self.provider_per_page)
        header_ = self.header_template
        header_["Referer"] = response.url
        priority_ = self.magic_number
        for sub_page in range(1, total_page_):
            url_ = (
                "http://www.foody.vn/ho-chi-minh/dia-diem?ds=Restaurant&vt=row&st=1&dt=undefined&c=3&"
                "page={}&provinceId={}&categoryId={}&append=true".format(
                    sub_page, self.provinceId, category_id
                )
            )
            yield Request(
                url=url_,
    ...
    def parse_first_comment(self, response):
        ...
        url_ = (
            "https://www.foody.vn/__get/Review/ResLoadMore?t=undefined&"
            "ResId={}&LastId=undefined&Count={}&Type=1&isLatest=true".format(resid, 5)
        )
        yield Request(
            url=url_,
            dont_filter=True,
            meta={"referer_url": response.url, "restaurant": restaurant},
    ...
    def parse_comment_detail(self, response):
        ...
        if len(comments) > 0:
            url_ = (
                "https://www.foody.vn/__get/Review/ResLoadMore?t=undefined&"
                "ResId={}&LastId={}&Count={}&Type=1&isLatest=true".format(
                    resid, lastid, 5
                )
            )
            yield Request(
